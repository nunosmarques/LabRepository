...
class CRUD extends LabApp{	
    ...
    function combox($tabela, $id){
		...
		if($id != NULL){
			while($result = $sql->fetch(PDO::FETCH_BOTH)){
				if($result[0] == $id){
					$retorno.= '<option selected value="'.$result[0].'">'.$result[1].'</option>';
				}else{
	...
	function select($table, $where_sign, $array_filter, $where_condition, $page_num) {
		...
		if(!empty($page_num) && $page_num != NULL){
			$page_num = "LIMIT 0, ".$page_num;
		}else{
			$page_num = "";
		}
		...
		if(sizeof($array_filter) > 0){
			...
			if($count > 0){
				...
				$query = "SELECT * FROM $table WHERE ".$string." AND deleted = 'N' ".$page_num."";
			...
			a:			
			$query = "SELECT * FROM ".$table." WHERE deleted = 'N' ".$page_num."";
    ...
    function select2($table, $where_sign, $array_filter, $where_condition, $page_num) {
        $result = array();
        $prep = array();
        $string = "";
        ...
        if(sizeof($array_filter) > 0){
            ...
            if($count > 0){
                ...
                $query = "SELECT * FROM $table WHERE ".$string.$page_num."";
                //echo $query;
                $sql = $this->conn->prepare($query);
