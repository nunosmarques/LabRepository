...
<form method="post" action="script_editar.php"  class="xpto" id="0" enctype="multipart/form-data">
    <fieldset>
        ...
        if(!empty($return[0][15]) && $return[0][15] != NULL && file_exists("imagens/lab/".$return[0][15])){
			echo '<label>Logo Atual:</label>
				<img src="imagens/lab/'.$return[0][15].'" width="20%" height="20%"/>';
		}
		echo '<label>Logo:</label>
        <input type="file" name="logo" id="logo" value="'.$return[0][15].'"><br>';
		
		if(!empty($return[0][16]) && $return[0][16] != NULL && file_exists("imagens/lab/".$return[0][16])){
			echo '<label>Foto Atual:</label>
				  <img src="imagens/lab/'.$return[0][16].'" width="20%" height="20%"/>';
		}
    ...
    <fieldset>
        <legend>Media </legend>';
		if(!empty($return[0][1]) && $return[0][1] != NULL){
