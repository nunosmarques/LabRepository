...
//INDIVIDOS
$query = "SELECT *
FROM laboratorioindividuo lab
INNER JOIN individuo ind on ind.idindividuo=lab.individuo_idindividuo
WHERE lab.laboratorio_idlaboratorio = '$id' AND  ind.aprovado='S' AND  ind.deleted='N'
ORDER BY lab.coordenador ASC";
//
//echo $query;
...
while($result = $sql->fetch(PDO::FETCH_BOTH)){
	...
	if(!empty($result[1]) && $result[1] != NULL && file_exists('imagens/media/'.$result[1])){
		$print2 .=  '<td><img border="0"  src="'."imagens/media/".$result[1].'" width="100" height="100"></td>
					</tr>';
	}else{
		$print2 .=  '<td><img border="0"  src="imagens/na.png" width="100" height="100"></td>
			</tr>';
	}
